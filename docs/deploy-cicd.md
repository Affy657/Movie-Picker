...
- **À chaque push / PR** : install, lint, build des deux apps (api + web).
...
1. **build-and-lint** : `pnpm install`, `pnpm run lint`, `pnpm run build`.
...
Le roadmap prévoit des tests (si présents). Pour l’instant, le workflow exécute uniquement **lint** et **build**. Dès qu’un script `test` sera ajouté (par ex. `pnpm run test` à la racine ou dans les apps), tu pourras l’ajouter dans le job `build-and-lint` :
