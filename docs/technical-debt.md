...
3. **La feuille de style de l'entrée est bloquante dans le `<head>`**, donc Overpass est déclarée quand la coquille est peinte. Sans ça, substitution de police tardive, taille de boîte changée, nouveau candidat LCP. La mettre **en ligne** dans le document satisfait la condition sur le papier et l'a pourtant fait perdre 2 à 3 points : voir l'impasse I9, le poids du document compte plus que l'aller-retour économisé.

Les trois préchargements de police portent `fetchpriority="low"` depuis le 2026-09-10, et cette condition est la raison de revérifier `home` après y avoir touché : à priorité haute, 53 Ko de police partaient dans la première vague et prenaient la bande passante de `react-vendor`. Mesuré sur le runner, le titre de la coquille reste le candidat retenu avec la priorité basse. Ne pas descendre plus bas sans remesurer `home`, une police arrivée après la première peinture change la taille de la boîte du titre.
...
Le même plugin fait deux autres choses depuis le 2026-09-10, et la première échoue aussi en silence :
...
- **I9 mettre la feuille de style de l'entrée en ligne dans le document.** Gagne 0,5 s de FCP en local et **zéro** sur le runner : 1,5 s de FCP avant comme après, sur les onze pages non prérendues. En échange, le document passe de 2 937 à 7 990 octets et `home` perd 2 à 3 points, son plus grand élément étant peint depuis le document lui-même (C2), avec un `elementRenderDelay` inchangé à 65 ms. Même verdict sur les pages prérendues, dont les styles de route ont suivi le même aller-retour : `/soutenir` passait de 45 à 85 Ko et son FCP de 1,5 à 1,8 s, pour un score inchangé. Leçon générale, et elle vaut contre l'intuition du « critical CSS » : sur ce runner, **le poids du document coûte plus que l'aller-retour qu'on économise**. Les styles doivent être dans le document, mais liés.
