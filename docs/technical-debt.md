...
- state: agent
- declencheur: observé le 2026-09-21 : le déploiement de production `096f50b0` a basculé les jobs sur le jeton OIDC (`gcloud scheduler jobs describe movie-picker-event-reminders --location europe-west1 --format='value(httpTarget.oidcToken.serviceAccountEmail)'` rend `movie-picker-scheduler@<PROJET_GCP>.iam.gserviceaccount.com`, et un `gcloud scheduler jobs run` a reçu un 200 de la révision). Le lot décrit dans `fix` peut partir ; seule réserve, la `piege` : une révision antérieure au 2026-09-21 ramenée par `rollback.yml` attend encore l'en-tête, donc garder le secret tant qu'une telle révision est dans le dépôt d'images (30 jours).
- impact: depuis le 2026-09-20 l'API accepte un jeton OIDC signé par `movie-picker-scheduler@` (`GoogleOidcSchedulerTokenValidator`, `SchedulerCallerAuthenticator`), `deploy.yml` ne monte plus `SCHEDULER_TOKEN` et passe les quatre jobs au jeton après chaque promotion. Deux restes, voulus pour ne pas ouvrir de fenêtre entre la fusion et le déploiement : les jobs sont encore créés par `deploy.yml` et non décrits en Terraform (les importer avant la bascule aurait mis la valeur de l'en-tête dans l'état, que l'identité de `plan` lit), et `SchedulerTokenValidator` lit encore `X-Scheduler-Token` quand `SCHEDULER_TOKEN` est configuré, ce que plus aucune révision ne fera après ce déploiement.
...
- fix: une fois le déclencheur observé, dans un seul lot : décrire les quatre jobs en Terraform (`google_cloud_scheduler_job`, `http_target` avec `oidc_token { service_account_email, audience }`, schedule `*/30 * * * *`, `15 3 * * *`, `45 3 * * *`, `0 10 * * *`, fuseau `Europe/Paris`, `attempt_deadline` 300 s, `retry_config { retry_count = 3, min_backoff_duration = "30s", max_backoff_duration = "300s" }` comme `deploy.yml` les pose depuis le 2026-09-21, URI `<URL run.app du service>/api/v1/scheduler/<passe>`) et les importer (`pnpm run terraform -- import 'google_cloud_scheduler_job.<nom>' projects/<PROJET_GCP>/locations/europe-west1/jobs/<job>`), retirer l'étape de `deploy.yml`, retirer `roles/cloudscheduler.admin` et le `serviceAccountUser` sur le planificateur de `module.ci`, supprimer `SchedulerTokenValidator`, `ISchedulerTokenValidator` et la branche `SharedToken` de `SchedulerCallerAuthenticator` avec leurs tests, retirer `SchedulerToken` de `MoviePickerOptions`, puis détruire le secret (`pnpm run terraform -- state rm 'module.api_secrets.google_secret_manager_secret_iam_member.accessor["SCHEDULER_TOKEN"]'`, idem pour `google_secret_manager_secret.this["SCHEDULER_TOKEN"]`, `gcloud secrets delete SCHEDULER_TOKEN`, retrait de `api_secret_names`) et le passage de `.env.example`.
...
## DEBT-042 les aperçus de partage d'une invitation sont génériques, l'option « aperçu riche » n'a plus d'effet

- state: agent
- impact: le lien d'invitation est `https://<front>/e/<slug>` et Hosting lui répond la coquille SPA, donc WhatsApp, Messenger, Discord et consorts affichent le titre et l'image génériques du site quelle que soit la soirée. L'endpoint `GET /api/v1/events/slug/{slug}/share-preview`, qui rend un HTML Open Graph de la soirée, n'est plus appelé par personne depuis `9a1ecc87` (2026-06-01, le lien de partage est passé de l'URL API à l'URL front) : `eventSharePreviewUrl` dans `apps/web/src/features/events/api/eventsApi.ts` n'a aucun appelant et le réglage `richSharePreview` de la soirée ne change rien pour l'utilisateur. Depuis la V1.7, le recap `/r/<slug>` a son aperçu : Hosting réécrit `/r/**` vers Cloud Run et `GET /r/{slug}` rend la coquille du site avec les balises de la soirée (`GetEventRecapDocumentHandler`, coquille lue sur `PUBLIC_WEB_BASE_URL/index.html` par `HttpWebShellSource`), ce qui est le mécanisme à reprendre pour l'invitation.
- ou: `apps/api-dotnet/MoviePicker.Api/Application/UseCases/EventSharePreview/GetEventSharePreviewHtmlHandler.cs`, `apps/api-dotnet/MoviePicker.Api/Application/UseCases/EventRecap/`, `apps/web/src/features/events/api/eventsApi.ts`, `infra/firebase-hosting.json` (réécritures)
- verify: encore ouvert tant que la commande ne renvoie pas une balise `og:title` propre à la soirée (la coquille rend le titre du site).
  ...
  ```
- fix: une réécriture Hosting `{ "glob": "/e/**", "run": { "serviceId": "movie-picker-api", "region": "europe-west1" } }` devant le repli `**`, et une route `GET /e/{slug}` de l'API qui rend la coquille avec les balises de l'invitation (titre, date, thème selon `richSharePreview`) sur le modèle de `EventRecapController`, image générique du site tant que la soirée n'a pas de film ; supprimer alors `share-preview`, `eventSharePreviewUrl` et `GetEventSharePreviewHtmlHandler`, dont `GetEventRecapDocumentHandler` reprend la description. Le 404 des soirées inconnues (DEBT-043) vient avec.
- piege: `robots.txt` interdit `/e/` : un robot d'indexation n'y va pas, seuls les robots d'aperçu (qui ignorent `robots.txt`) sont concernés. Tout document navigateur passe alors par Cloud Run (démarrage à froid possible sur chaque lien d'invitation, là où le recap n'expose qu'un lien partagé) : c'est le coût à peser avant de l'étendre. Les documents servis par l'API remplacent les en-têtes du middleware de sécurité (`Content-Security-Policy: frame-ancestors 'none'`, `Referrer-Policy`) par ceux de Hosting, sinon la coquille est bloquée par `default-src 'none'`.

## DEBT-043 toute adresse inconnue répond 200, et une route prérendue avec barre finale sert la coquille

- state: differe
- declencheur: la réécriture Hosting `/e/**` vers Cloud Run de DEBT-042, ou une page de la coquille vue indexée dans Search Console. Depuis la V1.7, `/r/<slug-inconnu>` répond déjà `404` avec la coquille (document rendu par l'API derrière la réécriture `/r/**`), la même voie vaut pour `/e/`.
...
- piege: Terraform ne peut pas modifier le gabarit d'un service que gcloud a déployé : gcloud nomme la révision dans le gabarit (`template.revision`), Terraform renvoie ce nom dans sa mise à jour et Cloud Run répond `409 Revision named '...' with different configuration already exists` (constaté le 2026-09-21 sur `max_instance_count`, `ignore_changes` cache la différence sans empêcher l'envoi). La sonde se pose donc d'abord par `gcloud run services update <service> --startup-probe httpGet.path=/health,httpGet.port=8080,periodSeconds=5,failureThreshold=12`, en recette puis en production, et le module la décrit ensuite pour un `plan` vide. Cette révision prend le trafic sans passer par la validation de `deploy.yml` (`traffic` est ignoré, le service est en `latestRevision`) : la poser juste avant un déploiement, jamais un soir de soirée.


## DEBT-048 la zone DNS n'a pas de CAA et sa politique DMARC n'observe rien

- state: agent
- bloque: la zone vit chez OVH, hors Terraform et sans CLI (`infra/README.md`, section Domaines) : chaque enregistrement se pose dans le manager, par Chrome, avec la session OVH de l'utilisateur ouverte et son accord explicite sur l'enregistrement (le mode automatique seul refuse une modification DNS, tentative du 2026-09-20 ; le TXT du second site Hosting a été posé ainsi le 2026-09-21, sur demande).
...
- state: humain
- bloque: un jeton fine-grained se crée dans l'interface GitHub, derrière le mode sudo (passkey ou code par courriel), que l'agent n'a pas le droit de franchir (tentative du 2026-09-21 : « Confirm access » sur `settings/personal-access-tokens/new`). Le reste est agent.
- impact: la valeur montée dans les révisions de production et de recette (`GITHUB_TOKEN=GITHUB_TOKEN:latest` dans les deux listes `SECRETS` de `deploy.yml`) est un jeton OAuth `gho_` de l'application « GitHub CLI », identique à `gh auth token` sur le poste, scopes `repo, workflow, gist, project, read:org` (constaté le 2026-09-21 par `curl -sS -D - -H "Authorization: Bearer $T" https://api.github.com/user`, en-tête `X-OAuth-Scopes`). L'API n'en a besoin que pour créer des issues et pousser des pièces jointes sur la branche `feedback-attachments` (`GitHubIssueClient.cs`). Depuis un conteneur compromis, ce jeton pousse sur master, réécrit les workflows et assume donc toutes les identités fédérées, `infra-apply` comprise.
...
- impact: l'apex `movie-picker.fr` porte deux TXT `hosting-site=` (celui du site `www` et celui du site legacy, posé le 2026-09-21 avant l'`apply` du second site) et Hosting le signale en `CD_CONFLICTING_CLAIMS` (« There must be at most one TXT record with the `hosting-site=` prefix »). Le domaine sert encore (`HOST_ACTIVE`, `OWNERSHIP_ACTIVE`), mais les quatre domaines du projet gardent un certificat de type `TEMPORARY` plus de trente heures après la bascule, là où Hosting remplace le temporaire par le certificat `GROUPED` du projet en quelques heures d'ordinaire ; le temporaire expire le 2026-12-19, et rien ne dit qu'il sera renouvelé tant que l'apex est en conflit. Le TXT du site legacy n'a rien prouvé : `web` est passé `OWNERSHIP_ACTIVE` quand son CNAME a visé `movie-picker-web-legacy.web.app`.
...
Trois nombres se tiennent et aucun n'est posé au même endroit : le pool est de 20 connexions par instance (`DefaultMaxConnectionPoolSize` dans `ServiceCollectionExtensions.cs`, gardé par `AddMoviePicker_MongoClientPool_FitsEveryCloudRunInstanceUnderTheClusterConnectionCap`), `maxScale` est à 5 sur le service Cloud Run depuis le 2026-09-21 (`max_instance_count` du module `cloud-run-api`, posé par gcloud puis décrit, jamais écrit par `deploy.yml` ; 20 auparavant), et le cluster accepte 500 connexions, dont une centaine à laisser aux opérateurs, à la sauvegarde nocturne et à la dev qui partage le cluster. Depuis le lot Terraform 8, la recette partage aussi le cluster : deux instances au plus (`environments/staging/main.tf`) et `maxPoolSize=10` dans son URI, soit 20 connexions prises sur cette centaine, jusqu'à ce que DEBT-053 déplace recette et dev sur le M0 d'un second projet Atlas ; `max_instance_count` de la production est à 5 depuis le 2026-09-21 (100 connexions au plus), 20 instances n'ayant jamais plafonné que le coût. Monter l'un sans baisser l'autre casse en silence : les instances au-delà du budget échouent à se connecter, la readiness passe rouge et l'alerte « base injoignable » part alors que la base va bien, précisément le jour de charge.
