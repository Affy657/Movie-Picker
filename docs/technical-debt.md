...
- impact: `X-Scheduler-Token` est un secret partagé de longue durée, le treizième de Secret Manager, tous lus en `:latest`. Le faire tourner demande deux gestes coordonnés (la version du secret, puis les quatre jobs Cloud Scheduler qui l'envoient), sans quoi les rappels de soirée cessent en silence, ce qui est déjà arrivé une fois avec un secret absent. Cloud Scheduler sait signer un jeton OIDC avec un compte de service ; l'API n'aurait qu'à vérifier l'émetteur Google et l'audience.
