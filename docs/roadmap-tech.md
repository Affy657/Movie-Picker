...
- ✅ ⚙️ `L` **CI/CD** : GitHub Actions — build, tests, image Docker API, déploiements. Livrés en continu sur master, puis **passés en déclenchement manuel** (voir V1.6) pour cause de quota de minutes.
...
**Objectif** : ramener la facture GitHub Actions sous le quota d'un dépôt privé, sans desserrer une seule porte de qualité.

- ✅ ⚙️ `S` **Déploiement manuel** (2026-09-10) : `.github/workflows/deploy.yml`, `workflow_dispatch` avec une cible `tout` / `front` / `api`. Un push sur master ne déploie plus, il ne joue que les portes de `ci-cd.yml`. Le job `verifier-ci` **consomme** ces portes au lieu de les rejouer : il interroge l'API GitHub et refuse de partir si le run `ci-cd.yml` du commit visé n'est pas vert, ce qui aurait coûté 27 minutes de plus par livraison. Trois conséquences en cascade : `sonar` perd son exclusion sur `workflow_dispatch`, devenue un trou puisqu'un job sauté ne fait pas échouer un run ; les longues listes de `needs` avec `always()` disparaissent au profit de `needs:` nus, ce qui supprime la propagation de skip qui avait laissé la prod dix jours en retard ; le filtre par chemin ne décide plus de ce qui se déploie, seulement de ce qui se vérifie. Mesure : un run master passe de 27 à environ 14 minutes-machine, et le chemin de déploiement ne coûte ses 13 minutes qu'une fois par livraison réelle au lieu d'une fois par commit. Contrepartie assumée : **la production est périmée par défaut entre deux déclenchements**, d'où le garde-fou final qui vérifie que chaque cible demandée est bien partie.
- ✅ ⚙️ `S` **Lighthouse hors du chemin de chaque push master** (2026-09-10) : le job pesait 9 des 27 minutes-machine d'un run master pour une mesure que seul un déploiement consommait. Il est passé dans `deploy.yml`, où il reste la porte bloquante de `deploy-front` avec ses seuils inchangés — la piste du cron hebdomadaire est écartée : un cron qui échoue ne bloque rien, et desserrer cette porte est déjà mesuré sans gain (Impasses I4 de [`technical-debt.md`](technical-debt.md)). Contrepartie : une régression de performance se voit au déploiement et non plus au push.
- ⬜ 🔒 `M` **Passage du dépôt en public** : les minutes GitHub Actions sont gratuites et illimitées sur un dépôt public, ce qui supprime la contrainte de quota et rend inutiles les deux items ci-dessus ; plusieurs intégrations aujourd'hui bloquées par le mode privé redeviennent accessibles. Prérequis non négociable : un audit complet de l'historique avant bascule, pas seulement des commits récents. Vérifier qu'aucun identifiant Atlas, GCP, AWS, Resend, TMDB ou VAPID n'a jamais transité par un commit (gitleaks sur la totalité de l'historique, pas sur le diff), qu'aucun `.env` n'a été versionné à un moment quelconque, et que les dumps de base et captures des dossiers RNCP ne contiennent pas de données personnelles réelles. Un secret trouvé dans l'historique impose sa rotation puis une réécriture d'historique, à faire avant la bascule et non après : une fois le dépôt public, tout ce qui a été exposé doit être considéré comme compromis.
