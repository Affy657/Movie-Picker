...
- **Statistiques utilisateur** : tableau de bord personnel sur la page profil — nombre de soirées **créées** / **rejointes**, films proposés, votes émis, films gagnants, éventuellement tags / thèmes les plus joués ; agrégats calculés côté API à partir des données existantes (events, participants, votes), pas de tracking supplémentaire.
- **Personnalisation de la couleur d’UI** : sélection d’une **palette parmi un préréglage** (ex. bleu, vert, violet, rose) en complément du mode sombre / clair ; persistance **locale** pour invité, **sur le compte** pour utilisateur connecté ; impact tech : tokens CSS (variables) côté front, pas d’upload de couleurs libres.
- **Avatar utilisateur (set préselectionné)** : choix d’une image parmi un **set fourni** (pas d’upload libre, évite la modération et le stockage d’images utilisateurs) ; affichage à côté du pseudo dans la soirée et le profil ; persistance sur le compte (champ `avatarId` ou équivalent côté API).
- **Connexion sociale (OAuth)** : connexion via **Google** (et éventuellement Apple, GitHub, Microsoft) en plus du couple email / mot de passe ; liaison à un compte existant si l’email correspond ; suppose librairie OAuth côté API .NET, secrets dédiés (client ID/secret par provider) et écran de consentement / mentions légales mis à jour.
- **Invitations in-app à une soirée** : envoyer une invitation **directe** à un autre utilisateur connecté (par pseudo ou email) en complément du lien de partage ; notification interne (badge, liste « Invitations reçues » dans Mes soirées) ; suppose recherche d’utilisateurs ou carnet de contacts léger, et règles anti-spam (limite d’invitations / utilisateur / jour).
- **Intégration Letterboxd** : connecter son compte Letterboxd pour importer la liste **« déjà vu »** (films watched) et éventuellement la **watchlist** ; alimente les marqueurs « déjà vu » et la recherche de films ; suppose API ou export Letterboxd, mapping vers IDs TMDB et gestion des écarts (films non trouvés). Hors scope : publication d’activité sur Letterboxd depuis Movie Picker.
- **Système de dons** : page de soutien au projet (ex. **Stripe**, **Buy Me a Coffee**, **Ko-fi** ou équivalent) ; lien depuis le footer ou la page profil, mention claire que c’est **facultatif** et **sans impact** sur les fonctionnalités ; pas de stockage de moyens de paiement côté projet (tout est délégué au prestataire) ; vérifier mentions légales / fiscales selon le statut retenu.
- **Deep links streaming** : sur la fiche film, lien direct vers l’**app native** (scheme mobile si dispo) ou le **site** du provider (Netflix, Prime Video, Disney+, etc.) lorsque TMDB *watch providers* indique une disponibilité ; complète l’affichage des **pastilles** prévues en V1 (un clic pour lancer la lecture quand c’est possible). Fallback web systématique si le scheme natif échoue.
- **Avertissements de contenu** : badges sur les fiches films pour signaler **violence**, **horreur**, **18+**, etc. via les *certifications* / *content ratings* TMDB ; **option hôte** « masquer les films 18+ » à la recherche / proposition pour les soirées familiales ; affichage neutre et factuel (pas de jugement éditorial).
- **Cercles d’amis** : groupes persistants d’utilisateurs (ex. « Les colocs », « La famille ») réutilisables d’une soirée à l’autre ; création / gestion par le propriétaire du cercle (ajout, retrait, renommage) ; **invitation en un clic** de tout le cercle à une nouvelle soirée ; vue « Soirées du cercle » avec historique commun. Levier fort de **rétention** ; suppose collection `circles` côté API et lien `circle ↔ event`.
- **Profil public léger** : page consultable par les autres utilisateurs (ex. `/u/:pseudo`) — pseudo, avatar, badges, stats publiques choisies par l’utilisateur ; **paramètres de visibilité** explicites (public / cercles uniquement / privé), **opt-in** par défaut côté privé ; aucune donnée sensible (email, soirées privées) exposée.
- **Mini-commentaires sur une proposition** : courte note (ex. ≤ 140 caractères) attachable à un film proposé ou voté (« vu y a 2 mois », « trop long ce soir », « VOSTFR uniquement ») ; visible par les participants de la soirée, à côté du film ; **modération basique** (suppression par l’auteur ou l’hôte) ; pas un chat de soirée — strictement lié à un film.
- **Badges / achievements** : récompenses visuelles automatiques selon l’usage (ex. « 10 soirées créées », « 100 votes », « Curateur horreur » d’après le thème le plus joué) ; affichés sur le **profil public léger** ; calcul côté API à partir des **stats existantes** (pas de tracking supplémentaire) ; éviter les badges qui poussent à un usage compulsif.
- **Bandeau de consentement (CMP)** : version **opérationnelle** des mentions cookies / confidentialité existantes — bandeau au premier accès avec **choix granulaire** (strictement nécessaires / mesure d’audience / éventuels tiers), persistance du choix, lien **« Modifier mes préférences »** dans le footer ; conditionne le **chargement effectif** de l’analytics et de tout SDK non strictement nécessaire (Sentry, etc. à arbitrer selon base légale). Aligné RGPD.
