...
- **Les utilisateurs** sont comptés au 12/09 : 21 comptes, lus dans le journal de la sauvegarde nocturne (`gh run view <id> --log`, ligne `moviepicker.users`), qui compte chaque collection de la base restaurée ; c'est la source à rejouer la veille de l'oral. Les chapitres 3 et suivants gardent 17 comptes, valeur du 05/09.
- **La diapositive 8** devient une carte probabilité par impact (primitive `.risk`) avec la liste des parades ; le point 4, transport des e-mails, est remplacé par la perte de la base, parée par la sauvegarde nocturne vérifiée livrée en 1.6.
- **Le relevé d'effort** de `01-planification.md` § 3.4 est recalculé au 12/09 sur `master` : 1 069 commits, 95 jours actifs, fenêtres de dates disjointes dont la somme est le total. Les chapitres 4 et suivants restent ancrés au 5 septembre (833 commits, 88 jours, 10 versions, 17 comptes) jusqu'à leur reprise.

### 16.2 Le chapitre 3 (C3.2.1), 12 septembre 2026

Quatre diapositives (9 à 12), reprises de l'ancien support et **réancrées au 12 septembre** sur décision du propriétaire (décision 10 bis : le 05/09 disparaît du support au fur et à mesure des reprises). Les faits, tous recalculés et reproductibles :

- **Surfaces GitHub** : 10 fiches (5 anomalies, 5 retours, toutes closes) ; 86 pull requests dont 35 fusionnées (26 humaines sur 27, 9 Dependabot sur 59) ; 851 exécutions de `ci-cd.yml` dont 540 sur master (`gh api repos/Affy657/Movie-Picker/actions/workflows/ci-cd.yml/runs?per_page=1&branch=master`, `.total_count`) ; 11 releases, la v1.6.0 du 12/09 comptée sur la fusion locale ; 160 tickets.
- **Activité** : 1 070 commits (`git rev-list --count master`), 187 fusions, 95 jours actifs sur 198 (27/02 au 12/09), 3,3 jours par semaine sur 29 semaines, 4,0 sur les 24 semaines actives, 5 semaines à zéro (toutes avant le 10 mai), série maximale 12 jours (1er au 12 septembre) ; commits par mois 1, 28, 72, 150, 227, 194, 127, 272 ; fusions 0, 0, 1, 6, 39, 52, 18, 71 ; 884 commits hors fusion dont 269 `fix` et 179 `feat` (ratio 1,5), 140 `docs`.
- **Cadence** : écarts entre versions 81, 6, 17, 8, 19, 17, 31, 10, 3, 5 jours ; médiane 14 (13,5), moyenne 19,7.
- **Stabilité de la chaîne sur master**, succès sur exécutions conclusives (annulées exclues) : mars 39 %, avril 69 %, mai 56 %, juin 54 %, juillet 94 %, août 78 %, septembre 67 % ; 64 % sur 529 depuis le 16/03. Septembre est compté **hors quinze exécutions qui n'ont jamais démarré** (échec en deux secondes, `steps: []`) : leur cause, le quota de minutes du dépôt privé atteint le 09/09, **ne se dit pas à l'oral, décision du propriétaire** ; les notes parlent d'exécutions « sans rapport avec le code ».
- **Qualité et sécurité** (SonarCloud public, 11/09) : couverture 88,1 %, Quality Gate OK, A / A / A, duplication 0,7 %, 0 bug, 0 vulnérabilité, 0 hotspot ; 0 alerte Dependabot et 0 alerte Code Scanning ouvertes (les 37 alertes CodeQL ont été traitées le 11/09). Disponibilité 100 % et erreurs 0,026 % restent les mesures du dossier Bloc 4, trente jours au 05/09, dites comme telles.
...
### 16.4 Thème 10, piloter seul, 12 septembre 2026

Diapositives 13 à 15 (C3.3.1), trois au lieu des quatre anciennes 14 à 17. La 13 fusionne les styles situés (quadrant) et l'affectation « à la main / confié à la chaîne » : le délégatif dominant et la colonne « confié à la chaîne » disaient la même chose. L'histogramme des commits par mission est retiré du support ; sa mesure est refaite au 12/09 dans `04` § A.1 (891 commits hors fusion, 70 sans préfixe, produit 24 %, fiabilité 49 %, chaîne 10 %, documentation 17 %) et reste une réponse de réserve. La 14 est l'ancienne 16 au titre raccourci. La 15 ajoute le fait qui change l'analyse : **la plus longue série du projet est désormais celle du 1er au 12 septembre, 12 jours, v1.6 et préparation de l'oral**, donc la posture d'août s'est répétée et la recommandation 2 (limite à 5 jours) n'a pas été appliquée ; se dit avant que le jury le trouve. Chiffres réancrés : juin 54 % (et non 52), chaîne 94 → 78 → 67 % (septembre hors quinze échecs sans rapport avec le code, cause à ne pas détailler). **Indicateur retiré** : « 2,7 → 6,1 commits par branche avant intégration » ; recalculé au 12/09 par `git rev-list --count m^1..m^2` sur chaque fusion, il dépend entièrement de l'inclusion ou non des fusions de synchronisation (`master` dans `v1.4`, 70 et 24 commits) et de la fusion de version, médiane à 1 dans les deux mois : pas robuste, donc retiré du support et de `04` § C.2. `REFS` 13 à 15 = C3.3.1. **Second passage, même jour, sur consigne « plein d'infos qui ne servent à rien »** : retirés les deux KPI de la 13 (54 → 94 % est déjà au thème 7 et dans les notes, les 3 canaux sont dans les notes), la ligne « Restitutions » (thème 11), les trois publics de la 14 (en notes, le commanditaire est au thème 11), les deux KPI FR / EN et Bloquante (fondus en une ligne « et sur le produit »), la ligne « Posture » de la 15 (fondue dans le résultat), la colonne d'indicateurs des recommandations et la recommandation 3 (déjà sur la diapo 12, dite en notes). Durées 1:10, 0:40, 0:50.

### 16.5 Thème 11, rendre compte et validation, 12 septembre 2026

Diapositives 16 à 18 (C3.4.1, la 18 aussi C3.4.2), trois au lieu des quatre anciennes 20 à 23. La 16 fusionne les deux commanditaires, les points de validation (une ligne : 11 versions datées, étiquetées sur le commit déployé, le tableau des contrôles bloquants n'est pas repris, il est au thème 2 et à la 13) et le compte rendu qui fait décider (trois niveaux, gabarit sur le cas des notifications, sans la barre 2 blocs / 3 blocs). La 17 garde les trois familles d'indicateurs et la boucle de 17 jours ; les trois retours et leurs statuts passent en notes. **Les mesures de satisfaction restent celles du 5 septembre** (14 soirées sur 19, 3 abonnements sur 17, 7 réponses, 100 %, 207 ms, 0,026 %), datées sur la diapo : l'accès MCP à Atlas est désactivé au niveau de l'organisation et la lecture de la chaîne de connexion est bloquée en mode auto, donc pas de recomptage au 12/09 ; à refaire si le propriétaire fournit le nombre de soirées menées au tirage sur 27. La 18 remplace les trois évolutions chiffrées de l'ancien bilan par le périmètre livré (11 versions, 106 items, 348 points) et le lot suivant réel, la 1.7 cadrée du 08 au 11/09, 7 items, 34 points ; la demande de validation vise « engager la 1.7 ». `06` réancré (21 comptes, 11 versions, 1.6.0 dans le tableau des points de validation, date des mesures explicite), `07` § 7 renvoie à la 18. `REFS` 16, 17 = C3.4.1 ; 18 = C3.4.1, C3.4.2. Durées 1:00, 0:50, 0:40.

### 16.6 Pas de commanditaire, pas de jalons du titre, 12 septembre 2026

Consigne du propriétaire, après le thème 11 : **le jury n'est pas un commanditaire, il n'y a pas de commanditaire, on ne parle plus des quatre échéances du titre, et ce qui, dans le barème, ne s'applique pas au projet ne donne lieu à rien** (pas de clause de style sur le handicap, le contexte international, le client). Appliqué au support, 18 → 16 diapositives : la 5 remplace le KPI « 4 / 4 échéances du titre » par « 3 versions correctives, sans conception » et sa légende « Restitution, release » par « Release » ; la 7 perd la colonne Commanditaire de la RACI et la chip Handicap ; la 9 remplace le point de vigilance 2 « lots documentaires » (indicateur = échéances du titre) par « périmètre livré hors du chiffrage initial, 58 sur 81, 72 % ⚠ », réalisé et non traité, légende « s'est réalisé » ; la 12 perd le bloc des modalités handicap ; l'ancienne 14 « Animer et inclure » est supprimée, ses quatre outils descendent sous le quadrant de la 13 et l'inclusion passe en notes « si on demande » ; l'analyse critique (14) dit « un dossier à rendre le 21/08 » ; les anciennes 16 et 17 fusionnent en 15 « Rendre compte aux utilisateurs, et mesurer », sans commanditaires ni gabarit décisionnel (le critère « faciliter la décision du client » ne s'applique pas, dit en notes) ; le bilan (16) demande de valider le périmètre livré sans « pour engager la 1.7 ». Chapitres alignés : `01` (rétroplanning sans dates imposées, RACI sans colonne Commanditaire, rôle Commanditaire retiré, jalons du titre retirés du Mermaid, risque 2 réécrit), `02` (indicateur « périmètre hors chiffrage » à la place des échéances tenues, § 3.4 réécrit, ligne Délais retirée), `03`, `04` (deux publics, comptes rendus aux utilisateurs), `06` (§ 1 réécrit, § 2.2 et gabarit § 3.2 à 3.4 retirés, deux niveaux de compte rendu, FAQ), `07`. Le chiffrage initial du cadrage (98 J/H dont un lot de clôture de 23) reste cité comme fait d'origine. `REFS` : 13, 14 = C3.3.1 ; 15 = C3.4.1 ; 16 = C3.4.1, C3.4.2.

### 16.7 Analyse critique en notes, bilan refait, 12 septembre 2026

Le propriétaire a jugé la diapositive d'analyse critique inutile : supprimée, 16 → 15 diapositives. Le thème 10 tient sur la seule 13 ; l'analyse (dix jours d'août, douze de septembre, chaîne 94 → 78 → 67 %, les deux règles) est dans ses notes en « si on demande », et `04` partie C reste la réserve. Deux critères de C3.3.1 (analyse critique, recommandations) ne sont donc plus portés par le support, seulement par les notes, choix du propriétaire, C3.3.1 n'est pas éliminatoire. Le bilan (15) est refait en trois blocs : **livré** (11 versions en 198 jours, 0 retour arrière ; 106 items, 348 points, 58 hors chiffrage ; 21 comptes, 27 soirées, 125 films au 12/09 d'après le journal de la sauvegarde nocturne ; 0 vulnérabilité, 0 anomalie ouverte, disponibilité 100 %), **retenu** (les trois enseignements, jusque-là en notes), **suite** (1.7 cadrée, 7 items, 34 points), puis la demande de validation. Les six verbes du parcours sont retirés. `REFS` : 13 = C3.3.1 ; 14 = C3.4.1 ; 15 = C3.4.1, C3.4.2.

### 16.8 Passe de pertinence sur les quinze diapositives, 12 septembre 2026

Consigne : retirer la phrase de demande de validation, puis supprimer sur tout le support ce qui n'est pas pertinent. Retiré : sommaire, la mention « éliminatoire » (méta) et « validation du périmètre livré » (entrée 11 devient « Rendre compte, et le bilan ») ; diapo 5, les KPI « 35 / 86 pull requests » et « 851 exécutions » (quatre surfaces restent), « bug corrigé en un jour » devient « 0 à 7 jours, cinq anomalies closes » ; diapo 6, le KPI « 3 versions correctives » (lisible sur le Gantt) ; diapo 7, l'histogramme des fusions et le KPI « 6 puis 39 », l'histogramme des commits passe en pleine largeur avec ses huit valeurs ; diapo 9, la ligne Licences ; diapo 11, le KPI « 12 routes » (déjà dans la frise) ; diapo 13, la ligne « management d'équipe » (hypothétique) ; diapo 15, la chip « adéquation au périmètre attendu » (pas de commanditaire, donc pas de périmètre attendu par un tiers) ; diapo 16, retitrée « Bilan », la phrase de demande de validation, les chiffres répétés dans « retenu » (les trois enseignements tiennent en une phrase chacun), et « 58 hors chiffrage » remplacé par « 81 produit, 25 techniques » pour ne pas le dire trois fois. Notes de la 16 : pas de demande, la validation est le jugement du jury sur ce qui a été démontré.

### 16.9 Annexes, 12 septembre 2026

Sept annexes reprises de `slides-v1.md`, diapositives 16 à 22, `ANNEX_FROM = 16`, triées par probabilité de question : A1 architecture (chiffres du 12/09 : 50 277 lignes C# non vides sur 652 fichiers, 88,1 %, A / A / A, duplication 0,7 % ; sauvegarde nocturne et promotion sans trafic ajoutées, livrées en 1.6), A2 arbitrages de réserve (juin réancré à **54 %**, comme au thème 7, `03` corrigé aux trois endroits qui disaient encore 52), A3 chaîne CI/CD (18 jobs recomptés sur `ci-cd.yml` et `deploy.yml`, 12 + 6 ; déploiement manuel par cible, révision sans trafic, build front archivé 30 jours, `deploy-guard` reformulé), A4 journal des versions (1.6.0 ajoutée, dates sans année), A5 retours utilisateurs, A6 RACI complète (quinze lignes de `01` § 4.2, sans colonne commanditaire, la primitive `.raci` du thème 5 telle quelle), A7 infrastructure palier par palier (ancien budget A3 réduit à ce que le thème 6 ne dit pas). **Retiré de A5, faute de source écrite** : « 6/7 ont voté au moins une fois », « 4/7 : ça dépend d'un réglage de l'hôte », « 5/7 : ça dépend des soirées », « 3/7 ont activé les notifications » ; ces quatre décomptes n'existent que dans `slides-v1.md`, ni dans `questionnaire-utilisateurs.md`, ni dans `axes-amelioration.md` § 2 bis, ni dans le dossier Bloc 4. Les six lignes restantes y sont toutes. Le propriétaire peut les remettre s'il les confirme sur les réponses du formulaire. Reste à réaligner : `00-plan-presentation-orale.md` § 1 et § 4, le script du § 5.3, les notes qui disent encore un numéro de diapositive.
...
**Diapositive 10, thème 8 « Un cas d'arbitrage »** (12/09) : reprise telle quelle de l'ancienne diapositive 13, frise, quatre options, cinq chiffres, logigramme ; mise à jour de deux chiffres, 1 → 53 (50 277 lignes non vides de C# sur 652 fichiers au 12/09, tests compris, contre 944 le 18/03) et 10 versions livrées sur le socle depuis. Les trois décisions à effet remesuré ne sont pas à l'écran, elles sont en réserve dans les notes pour les questions. Sommaire : « 8. Un cas d'arbitrage, C3.2.2 ». Durée 2:20.

**Diapositives 11 et 12, thème 9 « Les compétences »** (12/09) : d'abord reprises telles quelles des anciennes 18 et 19, puis **refaites** sur consigne du propriétaire, qui a jugé qu'une note de 0 à 4 sur des compétences n'avait pas de sens. La grille (primitive `.skills`) ne note plus rien : sept domaines (back, front, tests et qualité, accessibilité et performance, livraison et infrastructure, sécurité et exploitation, méthode) sur trois colonnes, **déjà acquis en février** (la pile du MVP, `v0.1.0` : Node, Express, Mongoose, Zod, React, Vite, Docker, aucun test), **appris sur le projet** avec la version qui le prouve (chaque case est une techno ou une méthode nommée, vérifiable dans `package.json`, un `.csproj`, un workflow ou `docs/roadmap.md`), **reste à acquérir** (feuille de route 1.7, 1.8 et backlog Terraform pour le technique ; indicateurs des thèmes 4, 7 et 8 pour la méthode). Le plan (`.skills.plan`) part de cette colonne orange : six lignes « d'où vient le besoin, compétence, moyen, preuve attendue », sans durée en jours ; recrutement et modalités handicap conservés en chips. **Correction du propriétaire, même jour** : il savait déjà en février C#, ASP.NET Core, hexagonal, driver Mongo et transactions, OpenAPI généré, xUnit, Playwright, SonarCloud, GitHub Actions, Dependabot regroupé et Gitleaks (déplacés en colonne « février », la migration de mars n'est donc pas un coût d'apprentissage, `05` § 1.2 corrigé) ; une case sans contenu reste vide plutôt que remplie de force ; s'il recrutait, ce serait un développeur front et designer et un product owner ou chargé de marketing, pour compléter son profil back et DevOps. `05-competences.md` § 2, § 3.2, § 3.3, § 4 et deux lignes de FAQ suivent (la cartographie et les quatre vagues du § 1 restent valables). Durées 1:50 et 0:50.
