...
```json
{
  ...
  "release": "7dab20c…",
  "dependencies": [{ "name": "mongodb", "status": "ok", "durationMs": 12 }]
}
```
...
Cinq politiques d'alerte sont configurées dans Cloud Monitoring. Les seuils sont volontairement placés **au-dessus du bruit mesuré** pour rester crédibles : une alerte qui se déclenche sans raison finit par être ignorée.
...
| **Erreurs 5xx anormales** | Réponses 5xx Cloud Run | > 5 sur 5 min | ERROR | Référence : 4 réponses 5xx en 30 jours |
...
- **Canal** : notification par e-mail vers l'adresse d'exploitation du projet, déclarée comme canal de notification Cloud Monitoring et rattachée aux quatre politiques.
...
| Politiques d'alerte et notifications e-mail | Google Cloud Monitoring | — | 0 € |
| Sentry | Developer (SaaS, région UE) | 5 000 événements/mois, rétention ~30 jours | 0 € |

Les sondes ajoutent environ 50 000 requêtes par mois sur l'API. Cloud Run étant facturé au temps de traitement (≈ 100 ms par appel `/health`), le surcoût reste dans l'offre gratuite ; effet de bord favorable : les instances restent tièdes, ce qui réduit les démarrages à froid pour les utilisateurs.
