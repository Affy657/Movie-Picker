...
- ✅ **Intégration Letterboxd** (2026-08-10) : synchronisation bidirectionnelle de la watchlist à partir du pseudo Letterboxd. Toutes les 6 h, les films ajoutés sur Letterboxd sont ajoutés dans Movie Picker (correspondance TMDB films et séries) et ceux sortis de la watchlist Letterboxd (vus, notés ou commentés) en sont retirés. Suivi par slug Letterboxd pour ne jamais toucher aux films ajoutés directement dans Movie Picker, et garde-fou qui bloque tout retrait si la lecture de la watchlist est incomplète. Bouton d'import immédiat avec écran de revue des correspondances, statut de dernière synchronisation et aide contextuelle en bulles cliquables sur la page Compte.
- ✅ **Sélection manuelle du film gagnant** (2026-08-11) : en alternative au tirage par la roue, l'hôte active un mode « choix manuel » — les cards de films entrent en animation de tremblement (repli statique si mouvement réduit), l'hôte clique sur le film choisi, puis l'animation de révélation habituelle se déclenche (sans rotation de roue), avec un badge « Choisi par l'hôte » sur le gagnant et une notification dédiée aux participants.
- ✅ **Flamme streak de soirées** (2026-08-12) : compteur de semaines consécutives durant lesquelles l'utilisateur a participé à une soirée où un film a été tiré (roue ou choix manuel), affiché en icône flamme + chiffre sur le profil public. Calculé à la volée sur l'historique des soirées terminées, en semaines ISO (lundi à dimanche, UTC) ; la semaine en cours ne casse jamais le streak tant qu'elle n'est pas terminée. La flamme s'affiche en évidence dans la carte d'identité du profil, colorée et animée tant que le streak est actif, grisée dès qu'il retombe à zéro. Le meilleur streak jamais atteint apparaît comme compteur dédié dans les statistiques.
- ✅ **Exclusion d'un film de la roue** (2026-08-11) : l'hôte peut, via le menu d'actions (trois petits points) d'une card film, exclure ce film du tirage sans le retirer de la liste de propositions ; le film reste visible, votable et commentable, mais affiché grisé pour signaler son exclusion. Un film exclu n'est ni tiré par la roue ni sélectionnable en choix manuel, et disparaît des segments de la roue. Réversible à tout moment par l'hôte, y compris après un tirage. Si tous les films sont exclus, le lancement est désactivé avec un message d'aide.
- ✅ **Connexion sociale (OAuth)** (2026-08-12) : connexion et inscription via Google ou GitHub en complément de l'email / mot de passe, par redirection serveur (API .NET). Liaison automatique à un compte existant si l'e-mail est vérifié par le fournisseur, création de compte sans mot de passe sinon. Page Compte : section « Connexions » pour lier ou délier un fournisseur, avec garde-fou empêchant de retirer la dernière méthode de connexion. Suppression de compte et définition d'un mot de passe adaptées aux comptes créés sans mot de passe. Mentions légales et politique de confidentialité ajoutées.
- ✅ **Système de dons** (2026-08-13) : page publique « Soutenir Movie Picker » (`/soutenir`, accessible sans compte, liée depuis le footer et le menu utilisateur) qui expose les frais réels du service et renvoie vers Ko-fi pour un don libre, ponctuel ou mensuel. Aucun paiement ne transite par Movie Picker et aucune fonctionnalité n'est réservée aux donateurs. Seule contrepartie, purement décorative : un badge « Soutien » sur le profil public, attribué automatiquement par webhook Ko-fi quand l'adresse e-mail du don correspond à celle du compte. Le webhook vérifie le token Ko-fi en temps constant, est idempotent par identifiant de notification, et ne conserve ni e-mail de donateur, ni montant, ni message : seule la date du premier don est stockée.
