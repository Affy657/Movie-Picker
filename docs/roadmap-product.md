...
- ✅ **Intégration Letterboxd** (2026-08-10) : import de la watchlist directement depuis le pseudo Letterboxd (lecture de la watchlist publique, recherche TMDB par titre/année, écran de revue des correspondances), import CSV en solution de repli pour les profils privés ; synchronisation automatique toutes les 6 h qui retire de la watchlist Movie Picker les films sortis de la watchlist Letterboxd (vus, notés ou commentés), avec suivi par slug Letterboxd pour ne jamais toucher aux films ajoutés directement dans Movie Picker.
