...
**Une clé de déduplication qui change rend invisibles les marqueurs de la révision précédente.** Les rappels de soirée se dédupliquent sur `EventReminderPass.OccurrenceKey` (soirée et heure de début) dans `push_dedup_markers` ; changer la forme de cette clé fait repartir, une fois, un rappel déjà envoyé par l'ancienne révision si le déploiement ou le retour arrière tombe dans sa fenêtre (une heure et un jour avant le début). Une version qui change une clé de déduplication se déploie la nuit, hors des fenêtres de rappel.
