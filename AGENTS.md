...
|---|---|---|
| Bouton | `Button`, `buttonClass` pour un `<Link>`, `LinkButton` | `variant` `primary` / `secondary` / `soft` / `ghost`, `tone` `default` / `danger` / `warning`, `size` `sm` / `md` / `lg`, `fullWidth`, `loading` ; un état actif se dit par la variante (`variant={open ? 'soft' : 'secondary'}`), pas par une classe qui redessine le bouton ; un libellé texte est centré optiquement par le bouton, jamais par un `<span>` de décalage |
| Attente hors d'un bouton | `Spinner` | `1em`, couleur courante, immobile en `reduced-motion` |
| Bouton à icône seule | `IconButton`, `iconButtonClass` pour un `<Link>` | `tone` `default` / `danger` / `onPoster`, `shape` `square` / `round`, `loading` |
| Retour vers la page parente | `BackLink` | `to` ou `onClick`, flèche et libellé ; jamais une classe `back-link` |
| Fenêtre modale | `Modal` | seul endroit du projet où `<dialog>` et `::backdrop` sont autorisés ; une modale à titre passe par `title` (+ `titleDetail`), jamais par une barre de titre recopiée |
...
| Bandeau flottant | `Notice` | `placement` `top` / `bottom`, `onClose` facultatif, actions en enfants |
| Pastille, badge, filtre, lien externe | `Chip` | tones `default` / `primary` / `success` / `warning` / `pending` / `danger` / `muted`, `dashed`, `dot`, quatre formes (étiquette, `onClick`, `onRemove`, `href`), centrage optique déjà intégré |
| Compteur, point d'état | `CountBadge`, `StatusDot` | `CountBadge` `tone` `primary` / `success` / `neutral`, `size` `sm` / `md` ; `StatusDot` `pulsing` ; jamais une pastille de compteur ni une `@keyframes` de pulsation locales |
| Surface de contenu | `Card` | `as` typé (`as={Link} to=…`), `padding` `none`/`sm`/`md`/`lg`, `radius` `md`/`lg`, `elevation` `none`/`sm`/`md`/`lg`, `surface` `default`/`sunken`, `interactive` ; un encart autonome `--color-surface-sunken` est une `Card surface="sunken"` |
| Page de formulaire seul | `FormPageShell` | retour, carte à barre d'accent, `<h1>`, description ; connexion, inscription, mots de passe, création de soirée |
...
| État de page | `EmptyState`, `ErrorState`, `InlineError`, `SignedOutState`, `Skeleton`, `ErrorBoundary` | `InlineError` pour un bloc qui n'a pas chargé dans une page, `ErrorState` pour la page entière ; un état vide passe par `EmptyState` (`compact` dans une rangée) |
| Titre de page, libellé en capitales | `typography.module.css` | `composes: pageTitle` sur tout `<h1>` de page ; `composes: overline` pour un surtitre, `composes: sectionLabel` pour l'intitulé d'un groupe ; seuls les badges posés sur une affiche (`posterBadge`, ruban du gagnant, étiquette de la démo) et les libellés mono de `/tech` (`monoLabel`) gardent leurs capitales propres |
...
Les props d'accessibilité portent le préfixe `aria` en camelCase sur toutes les primitives : `ariaLabel`, `ariaLabelledBy`, `ariaDescribedBy`, et le nom d'un sous-élément garde le suffixe (`closeAriaLabel`, `removeAriaLabel`, `panelAriaLabel`). `label` désigne toujours un texte visible (`Field`, `Tabs`, `Dropdown`, `InfoBubble`, `Tooltip`). Un composant qui étale les attributs natifs prend `data-testid` ; `Chip` et `Modal` l'acceptent aussi ; `ConfirmDialog` garde `testId` parce qu'il en dérive trois identifiants. Un `onChange` remonte toujours la valeur, `Toggle` et `ToggleRow` compris (`onChange(checked)`).

Deux règles de comportement, vérifiées par `check:architecture` :
...
Les contrastes des rôles de couleur sont testés par `apps/web/src/styles/foundationContrast.test.ts` : 4,5:1 pour le texte et 3:1 pour les contours de champ, en clair et en sombre, pour chacun des huit accents (texte d'accent sur la surface, la surface élevée, le fond, la teinte et le fond léger, accent sur la surface à 3:1), et sous `.on-dark` pour le texte d'accent ; la liste exacte des couples est dans `docs/design-system.md`. Un jeton de couleur modifié se valide par ce test, et un fond léger survolé (`--color-primary-soft-hover`) porte `--color-primary-text-hover`.
...
**Avant tout push sur master, toujours exécuter `pnpm run verify:local` et corriger toute erreur avant de push.** Obligatoire quelle que soit la conversation ou la feature. Dix-sept étapes en trois voies concurrentes, puis la suite front seule (`docs/technical-debt.md`, C11) : voie node (règles d'architecture, tests des scripts, `pnpm lint`, ESLint, Prettier), voie dotnet (`dotnet restore`, build Release avec `-warnaserror`, puis en parallèle `dotnet format --verify-no-changes`, tests API unitaires et d'intégration sur ce build, export OpenAPI et dérive des types), voie docker (workflows actionlint + shellcheck + zizmor, Terraform `fmt` + `validate`, Gitleaks sur l'arbre de travail, audit Trivy), enfin les tests front avec seuils de couverture. Le premier échec arrête tout ; une table des durées ferme le run.
...
**Pousser sur master ne déploie rien.** La mise en production est un geste manuel en deux temps, `gh workflow run deploy.yml --ref master -f stage=staging -f target=all` puis, la recette vérifiée, le même avec `-f stage=production` (cibles : `all`, `front`, `api`) ; la production refuse de partir si le run `ci-cd.yml` du commit visé n'est pas vert ou si la recette (`staging.movie-picker.fr`) ne sert pas ce commit. Une cible partielle (`front` ou `api`) est refusée dès que l'autre lane a changé entre la release que l'étape sert et le commit visé (comparaison GitHub dans `verify-ci`) : la sortie est toujours `target=all`. Ne jamais la déclencher sans demande explicite de l'utilisateur : le découpage existe pour qu'il groupe plusieurs livraisons dans un seul déploiement, héritage du temps où le dépôt était privé et ses minutes GitHub Actions facturées. Corollaire à annoncer en fin de tâche : **la production est en retard sur master par défaut**, et rien ne le signale.
...
**Retour arrière** : `gh workflow run rollback.yml --ref master -f stage=production` (API, révision Cloud Run antérieure à celle qui sert, ou `-f revision=<nom>` ; le workflow refuse une révision dont l'image a quitté le dépôt), `gh workflow run rollback-front.yml --ref master -f stage=production` (front, version Hosting précédente ou `-f commit=<sha>`, sans rien republier : Hosting garde les versions que `deploy-front` a étiquetées avec leur commit). Les deux prennent `stage=staging` : c'est en recette qu'un retour arrière se répète, un retour arrière jamais exercé n'est pas un filet. Les sondes web, les sondes de santé et l'authentification GCP sont des actions composites de `.github/actions/`, partagées entre déploiement et retour arrière : les modifier là, jamais en ligne dans un workflow. Les données ne reviennent pas en arrière avec le code : `docs/runbook-mongodb-restore.md` dit comment remettre une sauvegarde dans le cluster.

**Une identité GCP par environnement GitHub, et la révision sous la sienne.** Chaque workflow qui parle à GCP tourne dans l'environnement GitHub dont il assume le compte (`gcloud-auth`) : `production` et `staging` pour déployer et revenir en arrière, `backup` pour la sauvegarde (la seule identité qui lise l'URI de la base), `infra-apply` pour l'`apply` Terraform (la seule qui administre l'IAM, derrière un relecteur requis : le job attend un clic dans l'onglet Actions, à donner après lecture des deux résumés de plan qui le précèdent, jamais à laisser attendre), `infra-plan` pour le `plan` d'une pull request et les plans de master ; la fédération n'accepte chaque sujet que depuis `master`, sauf `infra-plan`. Un job ne peut assumer que le compte de son environnement, et un job qui construit (le front, l'image) ne porte pas `id-token: write` : ne jamais donner cette permission à un job qui exécute du code du lockfile, ni un environnement de déploiement à un job qui n'en a pas besoin. La révision Cloud Run tourne sous `movie-picker-api@<projet>`, qui ne peut que lire les secrets qu'elle monte (`--service-account` dans `deploy.yml`). Un nouveau secret dans `deploy.yml` demande aussi son `secretmanager.secretAccessor` sur ce compte, sinon la révision ne démarre pas et le déploiement s'arrête avant promotion. Les routes `/scheduler/*` n'attendent plus de secret partagé mais un jeton OIDC signé par `movie-picker-scheduler@` pour l'audience de l'API publique (`SCHEDULER_OIDC_AUDIENCE`, `SCHEDULER_OIDC_SERVICE_ACCOUNT`, posées par `deploy.yml`), que `GoogleOidcSchedulerTokenValidator` vérifie hors ligne contre les clés publiques de Google. Ce que Terraform ne voit pas, une liaison IAM posée à la main ou un compte hors description, `pnpm run check:iam` le relève dans la passe hebdomadaire.
...
**`master` reste à zéro issue SonarCloud et à A partout**, sur l'analyse de la branche entière et pas seulement du nouveau code : aucune issue ouverte, aucune issue acceptée ni déclarée faux positif dans l'interface SonarCloud, aucun hotspot de sécurité à revoir, A en fiabilité, sécurité, maintenabilité et revue de sécurité, 0 % de duplication, et une couverture globale au moins égale au plancher `COVERAGE_FLOOR` de `scripts/check-sonar.mjs`, relevé dès qu'elle monte et jamais baissé. Le job `sonar` le vérifie après le Quality Gate, qui ne juge que le nouveau code ; un écart fait échouer le run, donc le déploiement. `pnpm run check:sonar` lit la même chose depuis le poste pour la dernière analyse de `master` (`--branch <nom>` ou `--pull-request <numéro>` pour une autre). Une issue se corrige dans le code, jamais dans l'interface : l'accepter ou la déclarer faux positif la fait compter comme ouverte. Deux portes locales attrapent la plupart des règles avant le push : `eslint-plugin-sonarjs` (configuration `recommended`) sur les sources de `apps/web/src`, hors tests et fichiers exclus de l'analyse, dans `pnpm run lint` ; l'analyseur `SonarAnalyzer.CSharp` du projet API, avec `CA1822`, `CA1859`, `CA1861` et `SYSLIB1045` montés en avertissement dans `apps/api-dotnet/.editorconfig`, que le build `-warnaserror` de `verify:local` et de la CI refuse. Les règles que SonarCloud reprend d'autres extensions (accessibilité JSX, React, unicorn, assertions de test) ne tournent qu'en CI : après un push sur `master`, attendre le job `sonar` et corriger son étape avant toute autre chose si elle échoue.
...
- une arête entre features hors de `FEATURE_EDGES` (`scripts/check-architecture.mjs`), tests et `import()` dynamiques compris : `auth` ne dépend d'aucune feature, `movies` de `auth`, `events` et `letterboxd` de `movies`, `watchlist` de `letterboxd` et `events`, `profile` de `watchlist` et `events`, `notifications` de `auth` ; une feature nouvelle se déclare dans la table avant son premier import. Ce qui sert plusieurs features vit dans la plus basse qui en a besoin (`useWatchlist` et `watchlistApi` dans `movies`, `ProposeToEventModal` dans `events`) ou dans `shared/` ; les pages qui composent plusieurs features vivent dans `app/pages/` (`app/pages/account/`) ;
- en CSS : espacement (`px` comme `rem`, même mêlé à un jeton), `font-size` (hors jeton ou `clamp()` de jetons), `font-weight`, `line-height`, `letter-spacing`, `font-family`, `border-radius`, `z-index`, `opacity` hors `@keyframes`, durée, délai ou courbe de `transition` / `animation`, `outline-offset`, longueur d'un `filter` ou d'un `backdrop-filter`, largeur de bordure hors 1 px / 2 px, en valeur littérale ; le raccourci `font:` (hors `inherit`) ; une couleur littérale (hexadécimal, `rgb()`, `hsl()`, `oklch()` et les autres notations, nom de couleur quelle que soit sa casse) dans toute feuille, hors des déclarations de jetons de `01-foundation.css` et de `landingPalette.css` ; un `color-mix()` sur un jeton `--color-*` dans un module ; `outline: 2px solid var(--color-primary)` écrit à la main ; un `:focus-visible` qui retire le contour sans poser `--ring-focus` ; point de rupture hors échelle ; `<dialog>` ou `::backdrop` écrit hors de `Modal` ;
- en CSS toujours : une taille (`width`, `height`, `min-*`, `max-*`, `top`, `right`, `bottom`, `left`, `inset`), un `translate`, une piste de grille ou une propriété personnalisée de module littérale sous 96 px hors 1 px / 2 px ; un espacement littéral à l'intérieur d'un `calc()` ou d'un `clamp()` ; un `var(--jeton, repli)` sur un jeton de la fondation ; un jeton de la fondation que rien ne consomme ; un `:hover` hors de `@media (hover: hover)` ; une `animation` ou une `transition` de `transform` dont la classe n'a pas son pendant dans un bloc `prefers-reduced-motion`, règle par règle et non fichier par fichier ;
- en TypeScript : un `zIndex` numérique, même conditionnel, ou un `color-mix()` sur un jeton `--color-*` dans un objet `style`, le jeton se pose dans le module CSS ; une couleur littérale (`'#…'`, `rgb()`, `hsl()`, `oklch()`…) hors du logo des technologies, des boutons OAuth et du QR code, un canvas lit le jeton par `readCssToken` ; un nom de jeton (`'--x'`) lu depuis TypeScript et déclaré nulle part, seule une clé d'objet `style` ou un `setProperty` le déclare ; un `size={16}`, `iconSize={16}`, `width={16} height={16}` ou `size = 16` par défaut sur un composant, l'icône prend `ICON_SIZE.<pas>` ; un `ICON_SIZE` qui ne coïncide pas avec les `--icon-*` de la fondation, un `SIZE_PX` d'`Avatar` qui ne coïncide pas avec les `--avatar-*` ;
...
- une classe déclarée dans un `*.module.css` et utilisée nulle part, ou dans `styles/*.css` et absente de tout fichier TypeScript et de `index.html`. La règle résout le nom local de l'import fichier par fichier, suit les ré-exports (`export { styles as xStyles }`), et compte comme usage un `composes:` et un `:global(...)`, pas une position descendante (`.footer .btn` ne fait pas vivre `.btn`) ni un point dans un sélecteur d'attribut (`a[href$='.pdf']`). Les modules accédés par crochets (`styles[variable]`) sont inanalysables : ils sont exclus et **listés dans la sortie**, jamais passés en silence ;
- un `:global(.x)` dans un module dont la classe `x` n'est déclarée par aucune feuille globale, n'apparaît dans aucune chaîne d'un composant ni de `index.html` et n'est pas posée par une bibliothèque (`lucide`, `lucide-*`) : la surcharge est morte.
...
`pnpm run check:terraform` rejoue le job `lint-terraform` en local, dans `verify:local` : `terraform fmt -check` sur tout `infra/terraform/`, puis `init -backend=false -lockfile=readonly` et `validate` sur chaque module racine de `infra/terraform/environments/`, sans bucket ni identifiants. Terraform n'est pas installé sur le poste : tout passe par `pnpm run terraform -- <commande>`, qui lance l'image épinglée de `scripts/terraform.mjs` et fournit lui-même le bucket d'état (`TF_STATE_BUCKET`), le projet (`GCP_PROJECT_ID`) et un jeton gcloud d'une heure ; le binaire est épinglé à trois endroits que `check:tools` compare (`ci-cd.yml`, `scripts/terraform.mjs`, `required_version` de chaque racine), les providers sont suivis par Dependabot et leur montée demande `pnpm run terraform -- providers lock`. Le bucket d'état, seule ressource jamais décrite, se crée à la main : commandes dans `infra/README.md`. Aucun identifiant de projet ni de bucket ne s'écrit dans un fichier `.tf`.
...
**Une seule instance à la fois.** Avant d'exécuter une migration, le runner prend un bail de 30 minutes dans `migration_leases` (le document a pour `_id` l'identifiant de la migration) ; une instance qui trouve un bail vivant laisse la suite à son détenteur. Le bail d'une instance tuée expire seul, une migration qui dépasse 30 minutes peut donc tourner deux fois : c'est une raison de plus pour qu'`ExecuteAsync` reste idempotent.

**Faite seulement si complète.** Une migration par lots passe par `BackfillSteps.RunBatchesAsync` et rend `outcome.Completed(Id)` : un seul élément en échec lève `BackfillIncompleteException`, la migration n'est pas consignée et reprend au démarrage suivant, sur les seuls éléments restants. `RunBatchesAsync` relit la tête de la requête à chaque lot : quand un lot entier n'est fait que d'éléments déjà tentés (sautés ou en échec), il double la taille du lot au lieu de s'arrêter, faute de quoi cent éléments bloqués en tête cacheraient tout le reste.

**Rejouable quand la révision précédente réécrit derrière elle.** Une migration qui corrige une valeur que la révision encore servie continue d'écrire implémente `IRepeatableDataMigration` : le runner la joue à chaque démarrage, sous le même bail, sans jamais la consigner. Son filtre ne doit donc sélectionner que les documents à réécrire, pour que la plupart des démarrages ne trouvent rien. `RewriteLegacyPosterPathsMigration` en est une : elle ramène les anciennes clés d'affiche (`/api/v1/posters/<empreinte>`) sur la route `/api/v1/posters/tmdb/w500/<fichier>`, et `PosterReferences.ToStoredPosterPathAsync` fait la même conversion à l'écriture. Une fois qu'aucune révision servie n'écrit plus l'ancienne forme, une telle migration redevient ordinaire ou disparaît.

**Compatible avec la version d'avant.** Une migration tourne au démarrage de la révision, donc avant sa validation en recette et avant sa promotion, pendant que la révision précédente sert encore le trafic sur la même base. Elle ne fait qu'ajouter : un champ supprimé, renommé ou dont le sens change attend une version où plus aucune révision servie ne le lit. Même règle entre l'API et le front : `deploy.yml` publie l'API avant le front, et les navigateurs gardent l'ancien front en cache (service worker) plusieurs jours. L'API N sert donc le front N-1 : une route ou un champ qu'il lit ne disparaît pas dans N, et un champ ajouté à une requête reste optionnel côté API.

**Exception assumée : les affiches.** `RewriteLegacyPosterPathsMigration` change le sens de `posterPath`, et une révision qui ne connaît pas la route `/posters/tmdb/` répond 404 sur chaque affiche réécrite. Entre le démarrage de la nouvelle révision (sans trafic) et sa promotion, la production sert donc des affiches manquantes : une version qui porte cette migration se déploie à heure calme. Un retour arrière de l'API vers une révision antérieure à cette route laisse ces affiches vides jusqu'au déploiement suivant : ne pas revenir en deçà sans redéployer aussitôt.

**Une clé de déduplication qui change continue de lire l'ancienne.** Les rappels de soirée se dédupliquent sur `EventReminderPass.OccurrenceKey` (soirée et heure de début) dans `push_dedup_markers`, alors que les révisions d'avant le 2026-09-23 posaient leurs marqueurs sous l'identifiant de la soirée seul. Pour qu'un déploiement tombé dans une fenêtre de rappel (une heure et un jour avant le début) ne renvoie pas un rappel déjà parti, la passe consulte aussi le marqueur à l'ancienne clé, et seulement s'il date de l'ouverture de la fenêtre en cours (`INotificationDedupRepository.WasClaimedSinceAsync`) : un rappel envoyé pour la date d'avant un report ne bloque pas celui de la nouvelle date. L'inverse n'est pas couvert : une révision d'avant le 2026-09-23 ramenée par un retour arrière ne lit que l'ancienne clé, donc elle renvoie un rappel que la nouvelle a déjà envoyé dans la fenêtre en cours ; un tel retour arrière se fait hors des fenêtres de rappel des soirées ouvertes, ou accepte ce doublon. La notification « Soirée en suspens » suit la même règle depuis le 2026-09-24 : clé d'occurrence, lecture de l'ancien marqueur et de la boîte de réception limitée à ce qui date de l'ouverture de sa fenêtre (début plus `EventSchedule.PendingDelay`, `IUserNotificationRepository.ExistsSinceAsync`), pour qu'une soirée reportée puis de nouveau en suspens prévienne encore l'hôte. Une future forme de clé garde la même lecture de la précédente, sinon sa version se déploie la nuit, hors des fenêtres de rappel.
...
**Aucune erreur Mongo interceptée sous `IUnitOfWork.ExecuteAsync`.** Une écriture refusée (`DuplicateKey`, conflit de validation) fait avorter la transaction côté serveur : l'intercepter puis écrire de nouveau dans le même délégué échoue, et le pilote rejoue le délégué entier jusqu'à épuiser le budget de la transaction (15 s, `MongoUnitOfWork`), avant un `concurrent_update`. Un conflit attendu se lit avant d'écrire, ou se traduit en exception métier qui sort du délégué (`ParticipantConflictException`, rattrapée par `JoinEventHandler` hors transaction, qui rejoue avec un autre pseudo) ; `DeleteAccountRollbackTests` et `MongoTransactionTests` le vérifient contre Mongo.
...
- `infra/terraform` : description de l'infrastructure GCP en Terraform, un module racine par environnement (`infra/README.md`)
...
| SonarCloud | MCP `sonarqube` (Docker, requiert Docker Desktop lancé et l'image `mcp/sonarqube`) | consulter qualité / issues / hotspots ; l'analyse tourne en CI (job `sonar`, SonarScanner for .NET). Les règles SonarJS, complexité cognitive (`S3776`) comprise, tournent en local dans `pnpm run lint`, et `pnpm run check:sonar` lit l'état d'une branche par l'API publique, sans le MCP |
