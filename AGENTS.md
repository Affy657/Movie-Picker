...
**Une clé de déduplication qui change continue de lire l'ancienne.** Les rappels de soirée se dédupliquent sur `EventReminderPass.OccurrenceKey` (soirée et heure de début) dans `push_dedup_markers`, alors que les révisions d'avant le 2026-09-23 posaient leurs marqueurs sous l'identifiant de la soirée seul. Pour qu'un déploiement ou un retour arrière tombé dans une fenêtre de rappel (une heure et un jour avant le début) ne renvoie pas un rappel déjà parti, la passe consulte aussi le marqueur à l'ancienne clé, et seulement s'il date de l'ouverture de la fenêtre en cours (`INotificationDedupRepository.WasClaimedSinceAsync`) : un rappel envoyé pour la date d'avant un report ne bloque pas celui de la nouvelle date. Une future forme de clé garde la même lecture de la précédente, sinon sa version se déploie la nuit, hors des fenêtres de rappel.
