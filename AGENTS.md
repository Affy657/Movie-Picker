...
**Le document prérendu porte les styles de sa route**, liés dans son `<head>` d'après `dist/route-assets.json` (publié par le plugin de build, voir C4). Sans eux le contenu prérendu peint sans styles, se remet en page quand le chunk de la route arrive, et Chrome retient ce **second** rendu comme LCP : le prérendu ne rapporte alors rien, ce qui s'est mesuré le 2026-09-10 (`/soutenir`, `elementRenderDelay` de 585 ms sur un document déjà complet). Liés et non mis en ligne : l'impasse I9 a mesuré que grossir le document coûte plus que l'aller-retour économisé. Les balises sont posées de la plus profonde à la plus superficielle, la coquille avant la page, comme le fait le chargement par JavaScript.
