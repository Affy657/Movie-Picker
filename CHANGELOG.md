...
- **Le premier affichage est plus rapide sur toutes les pages** : les dépendances de l'ossature sont annoncées d'avance au navigateur au lieu d'être découvertes une fois son code exécuté, et les polices ne prennent plus la bande passante du code dont l'affichage dépend.
- **Les mentions légales et la politique de confidentialité sont servies en HTML complet**, comme les trois pages publiques déjà prérendues. Et chaque page prérendue déclare désormais ses styles dans son en-tête, donc son contenu s'affiche directement dans sa mise en page finale au lieu d'apparaître brut puis de se réorganiser.
