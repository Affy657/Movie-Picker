...
public sealed class EventSharePreviewTests : IClassFixture<MoviePickerApplicationFactory>
{
    ...
    [Fact]
    public async Task SharePreview_Default_IsGeneric_DoesNotLeakEventTitle()
    {
        ...
        var res = await anon.GetAsync($"/api/v1/events/slug/{created!.Slug}/share-preview");
    ...
    [Fact]
    public async Task SharePreview_Rich_IncludesEventTitle()
    {
        var client = await IntegrationTestAuth.NewRegisteredClientAsync(_factory);
        var create = await client.PostAsJsonAsync(
            "/api/v1/events",
        ...
        Assert.Contains("2035-08-01", html);
    }
}
