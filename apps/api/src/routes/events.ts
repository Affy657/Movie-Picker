...
async function eventToJson(event: IEventDoc, isHost: boolean): Promise<Record<string, unknown>> {
  ...
  const { hostToken: _omit, ...rest } = obj;
...
router.get('/slug/:idOrSlug', loadEvent, async (req: Request, res: Response, next: NextFunction) => {
  try {
...
router.post('/:idOrSlug/join', loadEvent, requireEventNotFinished, async (req: Request, res: Response, next: NextFunction) => {
  try {
...
router.post('/:idOrSlug/wheel', loadEvent, requireHost, requireEventNotFinished, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const event = req.event as IEventDoc;
    if (event.closedAt) {
    ...
    if (movies.length === 0) {
      res.status(400).json({ error: 'Aucun film proposé. Proposez au moins un film pour lancer la roue.' });
      return;
    }
    const winner = movies.length === 1
      ? movies[0]
...
router.post('/:idOrSlug/close', loadEvent, requireHost, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const event = req.event as IEventDoc;
    if (event.closedAt) {
