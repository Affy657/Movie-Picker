...
/** Affiche le même instant en fuseau local du navigateur (ou repli sur la chaîne brute). */
export function formatEventStartInUserTimezone(
  ...
  const loc = locale ?? (typeof navigator !== 'undefined' ? navigator.language : 'fr-FR');
  try {
...
export function computeEventStartReminder(
  ...
  if (terminé) return { visible: false };
