...
import EventStartReminderBanner from './EventStartReminderBanner';
...
describe('EventStartReminderBanner', () => {
  ...
  it('n’affiche rien hors fenêtre ou soirée terminée', () => {
    ...
    const { rerender } = render(
      <EventStartReminderBanner date="2030-12-01" time="21:00" terminé={false} />
    );
    expect(screen.queryByRole('status')).not.toBeInTheDocument();

    rerender(<EventStartReminderBanner date="2030-12-01" time="21:00" terminé />);
    expect(screen.queryByRole('status')).not.toBeInTheDocument();
  });
  ...
  it('affiche la bannière dans les 30 minutes avant le début (UTC stocké)', () => {
    ...
    render(<EventStartReminderBanner date="2030-12-01" time="21:00" terminé={false} />);
