 ...
 */
export default function EventStartReminderBanner({ date, time, terminé }: Props) {
  ...
  const state = computeEventStartReminder(date, time, terminé, Date.now());
  if (!state.visible) return null;

  return (
    <aside className="event-start-reminder" role="status" aria-live="polite">
      <strong className="event-start-reminder-kicker">Bientôt</strong>
      <p className="event-start-reminder-text">{state.line1}</p>
      <p className="event-start-reminder-meta hint">{state.line2}</p>
    </aside>
  );
}
