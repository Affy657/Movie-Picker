...
Rien d'autre : ni sonde ni politique d'alerte (le module `monitoring` n'est pas instancié), ni sauvegarde, ni workflow de retour arrière, un retour en recette est un redéploiement. Aucun service ne sort de son palier gratuit ; seules les versions de secret comptent (0,06 $ par version active et par mois au-delà des six gratuites), cinq de plus.
...
- **sur master**, `plan -out` puis `apply` de ce fichier, derrière l'environnement `production`, la production puis la recette (qui lit les sorties de la production) : ce qui est appliqué est exactement ce que le résumé du job montre. Un plan vide s'arrête là. Le même job se rejoue à la main (`gh workflow run infra.yml --ref master`) pour constater une dérive sans rien changer dans le dépôt.

Les deux chemins partagent l'action composite [`../.github/actions/terraform-plan`](../.github/actions/terraform-plan/action.yml) (installation, `init` sur l'état distant, `plan -detailed-exitcode` dont le code est la sortie de l'action, et sur master, avec `apply`, le résumé du plan sauvegardé puis son application). Ce qui en découle pour un lot : décrire, `plan` en local pour construire, puis pousser ; l'`apply` local reste possible (le compte propriétaire y lit et y écrit encore), et le job de master retrouve alors un plan vide. Tous les jobs partagent un groupe de concurrence : un `plan` de PR attend un `apply` en cours pour lire l'état qu'il laisse. Une liaison IAM fraîchement posée met une à deux minutes à être visible : un premier échec en `iam.serviceAccounts.getAccessToken` juste après un `apply` se rejoue.
